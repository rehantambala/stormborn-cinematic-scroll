...
const scene3LightningOverlay = document.getElementById('scene3-lightning-overlay');
...
let mouse = { x: 0, y: 0 };
let cursorTrail = [];
let scene3MassiveLightningPlayed = false;
...
function initThree() {
  ...
  // Dark storm lighting – low ambient, subtle blue
  ...
  scene.add(fillLight);
  // Flash light – turned on during lightning to illuminate whole scene
...
function createStormClouds() {
  ...
  for (let i = 0; i < layerCount; i++) {
    ...
    // Dark, subtle clouds (avoid bright/distracting background)
...
function triggerSceneFlash() {
  ...
  isFlashActive = true;
  const thorWrap = document.getElementById('thor-hero');
  if (thorWrap) {
    thorWrap.classList.add('flash');
    setTimeout(() => thorWrap.classList.remove('flash'), 160);
  }
  if (thunderFlash) {
    thunderFlash.style.opacity = '1';
    gsap.to(thunderFlash, { opacity: 0, duration: 0.2, ease: 'power2.out' });
  }
  if (flashLight) flashLight.intensity = 0.7;
...
function flashRandomLightning() {
  ...
  bolt.material.color.setHex(0xe8f4ff);
...
function runThunderFlash() {
  thunderFlash.style.opacity = '1';
  gsap.to(thunderFlash, { opacity: 0, duration: 0.25, ease: 'power2.out' });
}
...
// Transition helper: one cinematic strike between slides
function playTransitionStrike(intense = false) {
  // 3D lightning in the storm sky
  flashRandomLightning();
  if (intense) {
    // Occasional second bolt for extra drama in final slide
    setTimeout(() => flashRandomLightning(), 90 + Math.random() * 80);
  }

  // Global flash + Thor glow + exposure bump
  triggerSceneFlash();

  // Thunder overlay + subtle screen shake
  runThunderFlash();
  runScreenShake();

  // Diagonal bolt streak across the screen
  if (transitionBolt) {
    gsap.fromTo(
      transitionBolt,
      { opacity: 0, xPercent: -60 },
      {
        ...
        ease: 'power2.out',
        onComplete: () => {
          gsap.to(transitionBolt, { opacity: 0, duration: 0.08, ease: 'power2.in' });
        },
      }
    );
  }
...
function initScrollTrigger() {
  ...
  // Scroll-driven cinematic story: pin #story and fade scenes in sequence
  const tl = gsap.timeline({
    scrollTrigger: {
      ...
      onUpdate: (self) => {
        ...
        // Slight camera push-in over the story
        const z = 18 - progress * 4;
        camera.position.z = z;
        camera.lookAt(0, 0, 0);

        // Parallax intensity for clouds
        window.__scrollProgress = progress;
        window.__parallax = 0.5 + progress * 0.8;
      },
    },
  });
  ...
  scenes.forEach((scene, index) => {
    ...
    // Enter flash for scenes after the first (slide transitions only)
    if (!isFirst) {
      tl.call(() => {
        const isIntense = index === scenes.length - 1;
    ...
    // Fade out before moving to the next scene
    if (!isLast) {
      tl.to(
        scene,
        {
      ...
      // Final scene: allow a subtle zoom-in rather than fade-out
      tl.to(
        scene.querySelector('.scene-img'),
        {
          scale: 1.05,
          ease: 'power2.out',
        },
...
function triggerMassiveLightning() {
  if (!massiveLightningStrike) return;
  massiveLightningStrike.style.opacity = '1';
  massiveLightningStrike.style.background =
    'linear-gradient(180deg, transparent 0%, rgba(255,255,255,0.95) 35%, transparent 65%)';
  runScreenShake();
  flashRandomLightning();
  flashRandomLightning();
  const thorWrap = document.getElementById('thor-hero');
  if (thorWrap) {
    thorWrap.classList.add('flash');
    setTimeout(() => thorWrap.classList.remove('flash'), 220);
  }
  if (flashLight) flashLight.intensity = 1;
  renderer.toneMappingExposure = 1.4;
  bloomPass.strength = CONFIG.bloom.flashStrength + 0.5;
  if (thunderFlash) {
    thunderFlash.style.opacity = '1';
    gsap.to(thunderFlash, { opacity: 0, duration: 0.35, ease: 'power2.out' });
  }
  setTimeout(() => {
    if (flashLight) flashLight.intensity = 0;
    renderer.toneMappingExposure = 0.5;
  ...
  gsap.to(massiveLightningStrike, {
    opacity: 0,
    duration: 0.5,
    delay: 0.1,
    ease: 'power2.out',
...
function animate() {
  ...
  const scene3Intensity = scrollProgressVal > 0.6 ? 0.5 + (scrollProgressVal - 0.6) * 1.5 : 1;
  updateStormClouds(parallax * scene3Intensity, scrollProgressVal);
  ...
  // Only adjust bloom subtly during calm storm; strong flashes are handled in playTransitionStrike/triggerSceneFlash.
  if (!isFlashActive) {
    bloomPass.strength = CONFIG.bloom.baseStrength + Math.sin(t * 0.4) * 0.25;
  }
  composer.render();
}
