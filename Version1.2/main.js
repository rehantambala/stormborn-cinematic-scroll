/**
 * Thor Cinematic - Marvel Storm Experience (7 Scenes)
 * Three.js storm sky + GSAP ScrollTrigger + custom cursor
 * Smooth fade transitions between fullscreen scenes
 */
...
function flashRandomLightning() {
  ...
  if (!bolt) return;
  bolt.visible = true;
  bolt.material.opacity = 0.95 + Math.random() * 0.05;
  setTimeout(() => { bolt.visible = false; }, 80 + Math.random() * 60);
}
...
function triggerSceneFlash() {
  ...
  if (thunderFlash) {
    ...
    gsap.to(thunderFlash, { opacity: 0, duration: 0.3, ease: 'power2.out' });
  }

  if (flashLight) flashLight.intensity = 0.85;
...
function playTransitionStrike(intense = false) {
  ...
  if (intense) {
    // Occasional second bolt for extra drama in final scenes
  ...
  // Thunder overlay + subtle screen shake (reduced for smoother experience)
  runThunderFlash();
  
  // Only shake on intense transitions (scenes 5, 6, 7)
  if (intense) {
    runScreenShake();
  }
...
// --- GSAP ScrollTrigger for 7 Scenes ---
function initScrollTrigger() {
  ...
  // Calculate scroll distance: each scene gets equal space
  const scrollDistance = scenes.length * 1000; // 1000px per scene for smooth scrolling
  ...
  const tl = gsap.timeline({
    scrollTrigger: {
      ...
      onUpdate: (self) => {
        ...
        // Store progress for storm animation
  ...
  const segmentDuration = 1 / scenes.length; // Each scene gets equal timeline portion
  ...
  scenes.forEach((scene, index) => {
    ...
    const fadeInEnd = sceneStart + segmentDuration * 0.4; // Fade in over 40% of segment
    const fadeOutStart = sceneStart + segmentDuration * 0.6; // Fade out over last 40% of segment
    const sceneEnd = sceneStart + segmentDuration;

    // Lightning strike for scene transitions (except first)
    if (!isFirst) {
      tl.call(() => {
        // Intensify lightning on later scenes
...
function animate() {
  ...
  // Intensify storm on later scenes
  ...
  // Subtle bloom pulse during calm (not during flashes)
  if (!isFlashActive) {
    bloomPass.strength = CONFIG.bloom.baseStrength + Math.sin(t * 0.4) * 0.2;
  }
