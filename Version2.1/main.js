...
function triggerSceneFlash() {
  ...
  // Subtle bloom boost
  if (flashLight) flashLight.intensity = 0.3;
  renderer.toneMappingExposure = 0.7;
  bloomPass.strength = CONFIG.bloom.baseStrength + 0.5;
...
// Transition helper: smooth cinematic crossfade between scenes
function playTransitionStrike(intense = false) {
  // Subtle 3D lightning in the storm sky
  flashRandomLightning();
  if (intense) {
    // Occasional second bolt for final scene
...
function initScrollTrigger() {
  ...
  // Scroll-driven cinematic story: pin #story and crossfade scenes smoothly
  const tl = gsap.timeline({
    scrollTrigger: {
      ...
      onUpdate: (self) => {
        ...
        // Subtle camera push-in over the story
  ...
  scenes.forEach((scene, index) => {
    ...
    const fadeOutStart = start + segment * 0.55; // Crossfade overlap
    

    // Transition flash for scenes after the first
    if (!isFirst) {
      tl.call(() => {
    ...
    // Fade in this scene (crossfades with previous scene fading out)
    tl.to(
  scene,
  {
    opacity: 1,
    ...
    onStart: () => {
      ...
      const text = scene.querySelector('.story-text, .hero-text');
  ...
  fadeInStart
    ...
    // Fade out before next scene (overlaps with next fade in)
    if (!isLast) {
      tl.to(
        scene,
        {
